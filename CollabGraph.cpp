...
void CollabGraph::populate_graph(const vector<Artist>& artists) {
    ...
    //create vertexes in graph
    for(int i = 0; i < num_artists; i++){
        insert_vertex(artists.at(i));
    }
    //create edges where artists songs are the same
    for(int a = 0; a < num_artists; a++){
        for(int b = a + 1; b < num_artists; b++){
            string song = artists.at(a).get_collaboration(artists.at(b));
            if(song != ""){
                insert_edge(artists.at(a),artists.at(b),song);
            }
        }
    }
...
vector<Artist> CollabGraph::get_vertex_neighbors(const Artist& artist) const {
    ...
    vector<Edge> n = graph.at(artist.get_name())->neighbors;
    
    int neighbors_size = graph.at(artist.get_name())->neighbors.size();
    for(int i = 0; i < neighbors_size; i++){
        neighbors.push_back(n.at(i).neighbor->artist);
    }
    return neighbors;
}
...
stack<Artist> CollabGraph::report_path(const Artist& source,
    ...
    stack<Artist> path;
    stack<Artist> empty_path;
    
    //edge case if calling report path on itself, return the empty stack
    if(source == dest){
        return empty_path;
    }
    
    /*use the get predecessor to find all of the predecessors in the path
     and store them in a stack (from dest -> source)*/

    //push the dest
    Artist a1 = dest;
    path.push(a1);

    //loop until the predecessor is empty ""
    while(a1 != source){
        //push the predecessors
        a1 = get_predecessor(a1);
        path.push(a1);
        //return the empty path if there is no predecessor
        if(a1.get_name() == ""){
            return empty_path;
        }
    }
